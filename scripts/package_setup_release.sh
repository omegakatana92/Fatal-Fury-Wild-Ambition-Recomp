...
#
# Writes: dist/ffwa-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix ffwa \
  --exe-name Fatal_Fury_Wild_Ambition \
  --display-name "Fatal Fury Wild Ambition" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned FatalFuryWildAmbition disc" \
